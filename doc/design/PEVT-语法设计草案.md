...
- 目标事件通常位于另一个文件甚至另一个模组程序集里，静态阶段无法核对调用方实参与目标参数是否匹配——
  这件事只能在运行时晚绑定阶段核对，见 10.5 节与 `PEVTR4305`。
...
```pevt
resources "opening"("actor:aic:ixia/cmd-s134a-ixia-2", "sound:pr_down", "music:BGM_morinokioku")
```

规则：

- `resources` 是文件级声明，紧跟 `enable` 区域之后；出现在事件正文中间是静态错误（`PEVT1118`）。
- groupId 和全部资源引用都必须是双引号字符串字面量，不接受变量、表达式或其他动态值——
  预载要在加载阶段就开始，这一步不能等到运行时才知道该载什么。
- 一个文件可以声明零到多个组；同一文件中的 groupId 不能重复（`PEVT1117`）。
- 每条引用写成 `"<kind>:<内容>"`，`kind` 只能是 `actor`、`image`、`sound`、`voice`、`ambience`、`music` 之一：
  - `actor:<namespace>:<local-id>/<appearanceId>` 引用一个人物目录里已登记的 appearance；
  - 其余 `kind` 后面是对应服务认识的资源 ID（`image` 是事件图像 ID，`sound`/`voice`/`ambience`/`music` 是音频 ID）。
- 前缀不认识是 `PEVT1115`；前缀认识但剩余部分为空或形状不对（比如 `actor` 引用缺 `/appearanceId`）是 `PEVT1116`。
- 引用指向的资源是否真的存在是运行期事实（同一原因见 10.5 节 `callevt`：可能来自另一个模组），
  静态阶段只核对字面量形状，不核对目标是否存在。
- 声明的组一旦加载就立即为每个成员发起对应的加载请求；不等 `@wait_resources` 被调用。
- `@wait_resources("opening", timeoutFrames)` 对已声明的组直接使用这份预载票据集合等待；
  未声明过的 groupId 落回第一版的隐式规则（`.pactor` 人物 ID 或事件图像 ID）。
- 预载票据归当前事件所有；事件结束、替换或异常时的清理与其他资源票据一致。
...
```pevt
callevt "OtherEvent"(1, "forest")
handler h = callevt "AsyncEvent"(1)
```

- 实参表达式的求值、绑定与普通调用参数完全一致：未定义的名字、类型不匹配等仍按各自既有编号报告，
  这一步不因为出现在 `callevt` 参数位置而放宽或跳过。
- 实参数量、顺序与类型是否匹配目标事件声明的参数签名，**只能在运行时核对**——目标事件通常位于
  另一个文件甚至另一个模组程序集里，静态阶段没有机会看到它的参数声明。
- 运行时解析出目标事件后（10.4 节），无论调用是同步还是异步、目标是静态已知还是晚绑定得到的，
  都要立即核对实参与目标参数签名；数量或任一位置的类型不一致，产生运行时异常 `PEVTR4305`，
  且目标事件一步都不会执行。
- 核对通过后，实参在子事件环境中是调用时刻的值快照，子事件不能回写调用方的变量——语义与自定义事件块形参相同。
...
- `schedule` 在当前环境声明一个新的 `timelineId`，与 `frames` 帧之后启动 `call` 后面的目标。
  `timelineId` 只用于重复声明检测和调试展示，不能像 `handler` 那样被后续语句引用。
- `frames` 必须是 `int` 表达式；运行时求值结果为负数是运行错误。
- `call` 后必须是一个已经定义、声明为 `async` 且无参数的自定义事件块调用 `_名称()`；
  目标不满足这两条中任意一条都是加载期静态错误，不留到运行时才发现。
- `schedule` 本身不挂起当前流程：语句执行完立刻继续下一条语句，只是把动作排入队列。
- 到期时，目标块像被 `async block` 直接调用一样启动，成为当前事件拥有的普通异步协程；
  触发之后它和手写的 `async block` 调用没有任何区别，也同样可能在事件结束前从未被观察。
- `flush schedules` 立即启动当前环境全部尚未触发的项，不重复触发已经启动过的项。
- `clear schedules` 丢弃当前环境全部尚未触发的项，不影响已经启动的动作。
- 事件到达 `end`、被替换、异常或提前终止时，尚未触发的项一并丢弃——它们还没有产生任何效果，
  不需要像已启动的协程那样执行取消与清理。
- 调度队列按 `schedule` 语句求值 `frames` 时的 PolarisEvent 帧时钟推进，不依赖已经暂停的地图主循环。
