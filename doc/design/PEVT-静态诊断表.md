...
| 通用编号 | 实际发射的具体编号 |
| --- | --- |
| `PEVT5006` | 未定义名称 `PEVT6001`／不属于当前环境 `PEVT6002`／常量 `PEVT6010`／句柄 `PEVT7208` |
| `PEVT7202` | 词法上不可达：`async` 与 `block` 之间没有分隔符时两者会被词法器合并成单个标识符 `asyncblock`，因此实际报告的是 `PEVT1201`。解析器保留这道判断作为防御，但当前词法规则下不会触发。 |
