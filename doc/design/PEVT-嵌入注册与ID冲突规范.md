...
本节规定的是**分发**模型，与作者本机的开发路径无关：`PEVT-外部导入与热重载规范.md` 另有一条默认关闭的外部导入路径，允许在游戏里直接读作者目录下的 `.pevt` 并盖住同 ID 的嵌入版本。那条路径不改变本文的任何一条规则——它走同一套静态门，而且外部候选自成一个来源层级，不参与嵌入层的冲突判定，也不会让本文第 6.1 节的致命冲突消失。
...
`gzip-base64-v1` 的补充约定：

- 载荷固定为**单个** GZip 成员，不接受多成员串联；加载时按整个载荷的尾部读取 CRC32 与 ISIZE。
- `SourcePath` 判定为项目相对路径：非空、不以 `/` 开头、不含 `:`（盘符）、不含空段与 `..` 段。分隔符 `\` 与 `/` 等价，生成时统一写成 `/`。
- `ContentHash` 是未压缩 UTF-8 字节的 SHA-256 十六进制表示，比较时忽略大小写。它覆盖的是压缩前的原始字节；与程序定义里那个覆盖去 BOM 文本的 `SourceHash` 是两个不同的值。
...
任一校验失败时，该事件不进入可执行表；运行时保留来源程序集、相对源路径和具体诊断，但不尝试降级为原版 EV。第 1–4、6 步的失败使用静态诊断表新增的 `PEVT9201`–`PEVT9210`；第 5 步的失败直接保留语言核心自己的 `PEVTxxxx`，不再套一层包装编号。
...
- 解压后必须显式校验 GZip 尾部的 CRC32 与 ISIZE。`GZipStream` 读到被截断的流时不抛异常，而是返回已解出的部分字节；不做这一步，截断会被后面的长度或哈希检查笼统吃掉，作者拿到指向错误原因的诊断。
